...
# and draws a line from CENTER,CENTER to CENTER+x, CENTER+y
# also displays the X and y values
...
TEXT_X = adafruit_display_text.label.Label(terminalio.FONT, color=0x008080, text="x:")
TEXT_X.x = 12
TEXT_X.y = 3
g1.append(TEXT_X)
TEXT_Y = adafruit_display_text.label.Label(terminalio.FONT, color=0x808000, text="x:")
TEXT_Y.x = 12 if SIXTYFOUR else 0
TEXT_Y.y = 57 if SIXTYFOUR else 25
g1.append(TEXT_Y)
...
while True:
    ...
    f_x = round(f_x, 3)  # approx -10 to 10 where 0 = upright
    f_y = round(f_y, 3)  # approx -10 to 10 where 0 = sideways, 10 = upright
    f_z = round(f_z, 3)
